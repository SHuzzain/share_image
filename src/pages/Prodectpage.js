...
useEffect(() => {
   ...
   const newPost = posts.filter((prod) => {
       return prod.data().title.toLowerCase().includes(selector.searchValue.toLowerCase()) || prod.data().caption.toLowerCase().includes(selector.searchValue.toLowerCase())
  })
    //   if its get any post thst post array if true or else false
     ...
     const unSub =  onSnapshot(query(collection(db, 'posts')), orderBy('timestamp', 'desc'), async snapshot =>{
...
//  grid update
  ...
  return (
    ...
    <div className='w-full lg:w-[90%] p-4  '>
     <Masonry  breakpointCols = {oBjectBreackPorint}  className="flex  animate__zoomIn  animate__animated gap-9 ">
       {posts?.map((prod, index) => (
        <Prodect toggle = {handleDetailProdect} key={index} bookmarkCheck = {prod.data().bookmarkUserId.filter(userid => userid.userId === userIdSelector?.uid )} photoUrl = {prod.data().photoUrl} Nothover = {!token?.accessToken ? true : false} title ={prod.data().title} name = {prod.data().name} caption = {prod.data().caption} image={prod.data().imageUrl} id ={prod.data().uid} postId ={prod.data()._id} date = {prod.data().date}/>
       )) }
      </Masonry>
    </div>
    </div>
    ...
    <Loading color='yellow' type='spinningBubbles' width={60} height = {60}/>
    </div> ) : (
      <h1>{error.message}</h1>
    )
    }
