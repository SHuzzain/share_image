...
import {useRouteError , useParams, json, isRouteErrorResponse, useNavigate } from 'react-router-dom'
   ...
   useEffect(() => {
     ...
     const unSub = onSnapshot(particulerData, async snapshot => {
        ...
        if(await snapshot.docs[0].data().bookmarkUserId.length > 0){
           const checkUserIn = await snapshot.docs[0].data().bookmarkUserId.map(val => val.userId === userIdSelector?.uid)
           if(checkUserIn[0]){
     ...
     try {
      ...
      case 'remove':
          try {
             ...
             const removeUid = await useridDocs.docs[0]?.data().bookmarkUserId.filter(userid => (
              userid.userId === userIdSelector?.uid 
              ))
  ...
  return (
    <div className='p-4 h-full w-full  flex justify-center items-center'>
      ...
      <article className='bg-slate-300 p-4 max-lg:flex-col  rounded-sm max-w-[70rem] min-w-[50rem]  max-lg:max-w-[700px]  max-lg:min-w-[10rem]  capitalize font-semibold text-xl flex gap-2'>
        ...
        <img  className='w-full h-full object-contain rounded-md' src={selectedPost.imageUrl} alt="" />
        </section>
        <section className='flex lg:flex-[0.6] flex-col relative justify-center text-[16px] font-medium '>
          <h4>username: <span>{selectedPost.email}</span></h4>
          <h4>image uploder: <span>{selectedPost.name}</span></h4>
          <h4>title: <span>{selectedPost.title}</span></h4>
          <h4>caption: <span>{selectedPost.caption}</span></h4>
          <h4 >publiched on: <span>{selectedPost.date.slice(0,10)}</span></h4>
          <h4 className='absolute bottom-0 right-0'>{bookmarkCheck ? 
         <button> <CheckIcon onClick={() => handlepostToggle('remove')} aria-details='svg'  className='w-14 max-[1400px]:w-10 max-[1400px]:h-8 h-9 cursor-pointer  rounded-lg font-bold  bg-white text-gray-500'/> </button>
           : 
         <button> <PlusIcon onClick={() => handlepostToggle('add')} aria-details='svg'  className='w-14 max-[1400px]:w-10 max-[1400px]:h-8 h-9 cursor-pointer  rounded-lg font-bold  bg-white text-gray-500'/> </button>
          }</h4>
        </section>
      </article>
      ) : (
